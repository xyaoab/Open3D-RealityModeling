...
__global__ void VertexAllocationKernel(
    ...
    if (server.N_ >= 3) {
        if (Xlocal(0) < 3 && Xlocal(1) < 3 && Xlocal(2) < 3) {
            tsdf_volume.CacheNeighborSubvolumes(Xsv, Xlocal - Vector3i::Ones(),
        ...
        for (int i = 0; i < 27; ++i) {
            tsdf_volume.CacheNeighborSubvolumes(
                    ...
                    neighbor_subvolume_indices, neighbor_subvolumes);
        }
    }
    __syncthreads();
...
__host__ void ScalableMeshVolumeCudaKernelCaller::VertexAllocation(
        ScalableMeshVolumeCuda &mesher, ScalableTSDFVolumeCuda &tsdf_volume) {
    const dim3 blocks(mesher.active_subvolumes_);
    const dim3 threads(mesher.N_, mesher.N_, mesher.N_ / 4);
    VertexAllocationKernel<<<blocks, threads>>>(*mesher.device_,
                                                *tsdf_volume.device_);
    CheckCuda(cudaDeviceSynchronize());
    CheckCuda(cudaGetLastError());
}

__global__ void VertexExtractionKernel(
        ScalableMeshVolumeCudaDevice server,
        ScalableTSDFVolumeCudaDevice tsdf_volume) {
    __shared__ UniformTSDFVolumeCudaDevice *neighbor_subvolumes[27];
    __shared__ int neighbor_subvolume_indices[27];
    ...
    for (int workload = 0; workload < 4; ++workload) {
        ...
        if (tsdf_volume.OnBoundary(Xlocal, true)) {
            server.ExtractVertexOnBoundary(Xlocal, subvolume_idx, Xsv,
                                           tsdf_volume, neighbor_subvolumes);
        } else {
            server.ExtractVertex(Xlocal, subvolume_idx, Xsv, tsdf_volume,
                                 neighbor_subvolumes[13]);
        }
    }
}

__host__ void ScalableMeshVolumeCudaKernelCaller::VertexExtraction(
        ScalableMeshVolumeCuda &mesher, ScalableTSDFVolumeCuda &tsdf_volume) {
    const dim3 blocks(mesher.active_subvolumes_);
    const dim3 threads(mesher.N_, mesher.N_, mesher.N_ / 4);
    VertexExtractionKernel<<<blocks, threads>>>(*mesher.device_,
                                                *tsdf_volume.device_);
    CheckCuda(cudaDeviceSynchronize());
    CheckCuda(cudaGetLastError());
}

__global__ void TriangleExtractionKernel(
        ScalableMeshVolumeCudaDevice server,
        ScalableTSDFVolumeCudaDevice tsdf_volume) {
    __shared__ UniformTSDFVolumeCudaDevice *neighbor_subvolumes[27];
    __shared__ int neighbor_subvolume_indices[27];
